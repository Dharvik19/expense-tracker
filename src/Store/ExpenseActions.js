    ...
    const getExpenseFetching = async (email) => {
      try {
        const response = await fetch(
          `https://expensetracker-fdd18-default-rtdb.firebaseio.com/expenses/${email}.json`,
          {
            method: "GET",
            headers: {
...
//https://expensetracker-fdd18-default-rtdb.firebaseio.com//${userID}.json`
...
export const editExpenseFetching = (expenseItem, email) => {
  return async (dispatch) => {
    const editExpenseFetching = async (expenseItem, email) => {
      try {
        const response = await fetch(
          `https://expensetracker-fdd18-default-rtdb.firebaseio.com/expenses/${email}/${expenseItem.id}.json`,
          {
            method: "PUT",
            body: JSON.stringify({
...
export const deleteExpenseFetching = (id, email) => {
  return async (dispatch) => {
    const deleteExpenseFetching = async (id, email) => {
      try {
        const response = await fetch(
          `https://expensetracker-fdd18-default-rtdb.firebaseio.com/expenses/${email}/${id}.json`,
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
